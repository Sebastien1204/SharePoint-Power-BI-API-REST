...
type
  TForm1 = class(TForm)
    ...
    lblLignesAAjouter: TLabel;
    lblRowID: TLabel;
    lblRowName: TLabel;
    lblRowSurname: TLabel;
    edtRowID: TMemo;
    edtRowName: TMemo;
    edtRowSurname: TMemo;
    btnGetToken : TButton;
    ...
    procedure btnAddRowsDatasetClick(Sender : TObject);
  private
    AccessToken : string;
    Sites : string;
    Directories : string;
...
begin
  ...
  begin
    ...
    edtClientID.Text := 'f5ba69cd-35bf-47a6-b4e3-69d2b9556553';
    edtClientSecret.Text := '~rq8Q~ePesYBa7VGXvgY1wECxk53EztwhF6EUb9l';
...
procedure TForm1.btnAddDatasetClick(Sender: TObject);
begin
  if AccessToken.IsEmpty then
  begin
    ShowMessage('Veuillez d''abord récupérer le token.');
  ...
  begin
    ShowMessage('Veuillez entrer un nom de jeu de données valide.');
    Exit;
  end;
...
begin
  ...
  try
    ...
    // Construire le contenu JSON dynamiquement
    JSONContent :=
    '{' +
      '"name": "' + edtDatasetAAjouter.Text + '",' +
      '"defaultMode": "Push",' +
      '"tables": [' +
        '{' +
          '"name": "Table1",' +
    ...
    RESTRequest.AddBody(JSONContent, ctAPPLICATION_JSON);
...
procedure TForm1.btnAddRowsDatasetClick(Sender: TObject);
begin
  if AccessToken.IsEmpty then
  begin
    ShowMessage('Veuillez d''abord récupérer le token.');
  ...
  if (Trim(edtRowID.Text) = '') or (Trim(edtRowName.Text) = '') or (Trim(edtRowSurname.Text) = '') then
  begin
    ShowMessage('Veuillez remplir tous les champs : ID, Name, et Surname.');
    Exit;
  end;
...
procedure TForm1.AddRowsDataset;
var
  RESTClient: TRESTClient;
  RESTRequest: TRESTRequest;
  RESTResponse: TRESTResponse;
...
begin
  ...
  Endpoint := Format('https://api.powerbi.com/v1.0/myorg/groups/%s/datasets/%s/tables/Table1/rows', [edtWorkspaceID.Text, edtDatasetID.Text]);
  ...
  try
    ...
    // Construire dynamiquement le contenu JSON
    JSONContent :=
      '{' +
        '"rows": [' +
          '{' +
            '"ID": ' + edtRowID.Text + ',' +
            '"Name": "' + edtRowName.Text + '",' +
            '"Surname": "' + edtRowSurname.Text + '"' +
          '}' +
        ']' +
      '}';
    ...
    begin
      MemoOutput.Lines.Add('Le jeu de données a été modifié avec succès.');
    end
    else
    begin
      MemoOutput.Lines.Add('Erreur lors de la modification du jeu de données :');
      MemoOutput.Lines.Add(Format('Code HTTP : %d - %s', [RESTResponse.StatusCode, RESTResponse.StatusText]));
      MemoOutput.Lines.Add('Réponse : ' + RESTResponse.Content);
    end;
  except
